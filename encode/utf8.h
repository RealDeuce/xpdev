...
XPDEV_ENCODE_EXPORT int utf8_decode_firstbyte(char ch);

// Returns true if the string is valid UTF-8
XPDEV_ENCODE_EXPORT bool utf8_str_is_valid(const char*);

// Returns the fixed printed-width of the UTF-8 string
XPDEV_ENCODE_EXPORT size_t utf8_str_total_width(const char*, size_t zerowidth);

// Return the count of chars within the specified width range in UTF-8 string (str)
XPDEV_ENCODE_EXPORT size_t utf8_str_count_width(const char*, size_t min_width, size_t max_width, size_t zerowidth);

// Like strlcpy(), but doesn't leave a partial UTF-8 sequence at the end of dst
XPDEV_ENCODE_EXPORT size_t utf8_strlcpy(char* dst, const char* src, size_t size);

// Normalizes (to ASCII) chars in UTF-8 string 'str', in-place, resulting in string <= original in length
XPDEV_ENCODE_EXPORT char* utf8_normalize_str(char* str);
...
XPDEV_ENCODE_EXPORT char* utf8_replace_chars(char* str, char (*lookup)(enum unicode_codepoint), char unsupported_ch, char unsupported_zwch, char error_ch);
...
XPDEV_ENCODE_EXPORT int cp437_to_utf8_str(const char* src, char* dest, size_t size, unsigned char minval);
XPDEV_ENCODE_EXPORT int utf8_to_cp437_str(const char *src, char *dest, size_t size, unsigned char minval, size_t *outlen);
...
XPDEV_ENCODE_EXPORT int latin1_to_utf8_str(const char* str, char* dest, size_t size, unsigned char minval, size_t *outlen);
XPDEV_ENCODE_EXPORT int utf8_to_latin1_str(const char *src, char *dest, size_t size, unsigned char minval, size_t *outlen);

// Decode a UTF-8 sequence to a UNICODE code point
XPDEV_ENCODE_EXPORT int utf8_getc(const char* str, size_t len, enum unicode_codepoint* codepoint);

// Encode a UNICODE code point into a UTF-8 sequence (str)
XPDEV_ENCODE_EXPORT int utf8_putc(char* str, size_t len, enum unicode_codepoint codepoint);
